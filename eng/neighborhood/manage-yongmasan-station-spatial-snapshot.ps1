...
param(
    ...
    [string] $OutputPath = 'artifacts/local/yongmasan-station-spatial-snapshot/private-review.json',
    [string] $AuditOutputPath = 'artifacts/local/yongmasan-station-spatial-snapshot/coverage-audit.json'
)
...
function Require($condition, [string] $code) {
    if (-not $condition) { throw "YongmasanStationSpatialSnapshotManager:$code" }
}
...
$builder = Join-Path $PSScriptRoot 'build-yongmasan-station-spatial-snapshot.py'
$auditor = Join-Path $PSScriptRoot 'audit-yongmasan-station-spatial-snapshot.py'
$acquisition = Join-Path $PSScriptRoot 'acquire-yongmasan-station-osm.ps1'
