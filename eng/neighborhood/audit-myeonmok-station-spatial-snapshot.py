#!/usr/bin/env python3
"""면목역 공간 사본을 동결 원천 재구성과 불변식으로 검사한다."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--snapshot",
        default="artifacts/local/myeonmok-station-spatial-snapshot/private-review.json",
    )
    parser.add_argument(
        "--audit-output",
        default="artifacts/local/myeonmok-station-spatial-snapshot/coverage-audit.json",
    )
    parser.add_argument(
        "--building-zip",
    ...
    parser.add_argument(
        "--osm",
        default="artifacts/local/neighborhood-source-acquisition/myeonmok-station-0721-r1/map.osm",
    )
    parser.add_argument(
        "--osm-receipt",
        default="artifacts/local/neighborhood-source-acquisition/myeonmok-station-0721-r1/receipt.json",
    )
...
def main() -> int:
    ...
    try:
        ...
        descriptor = myeonmok_descriptor(
        ...
        print(f"MyeonmokStationSpatialSnapshotAuditFailed:{exc}", file=sys.stderr)
        return 1
