...
| 기획 ID | 현재 문서 | 상태 | 역할 |
| --- | --- | --- | --- |
| `PLAN-OPERATIONS-NEIGHBORHOOD-MICRO-HUB` | [생활권 소형 물류거점 r1](Planning/공통/PLAN-OPERATIONS-NEIGHBORHOOD-MICRO-HUB/README.md) | `Approved / FirstSimulationSliceImplemented / OperationalActivationDeferred` | 독립 비주거 유휴 공간을 Mongo 지도 신청 가원장과 결속하고 세 주체의 동의·승인 뒤 Simulation에서 상온 밀봉품 단기 보관·인계·모의 보상을 검증한다. 실제 보관 운영과 지급은 계속 비활성이다. |
| `PLAN-OPERATIONS-ABNORMAL-WORK-RECOVERY` | [비정상 업무 처리·회복 r5](Planning/공통/PLAN-OPERATIONS-ABNORMAL-WORK-RECOVERY/README.md) | `Approved / FirstPartialDamageSliceImplemented / OperationalActivationDeferred / FifthDecisionAsked` | 정상 8개 인수·파손 2개 영향분 보류, 운영자 revision 결정과 보험 적용 가능성 검토 상태를 구현했다. 다음 문답은 수령인 부재의 재연락·대기·운영자 선택이다. |
| `PLAN-OPERATIONS-ORDER-CENTERED-WORK-NETWORK` | [주문 중심 운영체제 업무망 r6](Planning/공통/PLAN-OPERATIONS-ORDER-CENTERED-WORK-NETWORK/README.md) | `Draft / Priorities1To5FirstSliceImplemented / Priority6Partial / RootAuthorityPending / ImplementationInProgress` | 개인 주문자는 본인, 공동주문은 대표를 기본 화주로 둔다. 운송 건의 판본화된 주·보조 담당자 권한과 화주→화물→화주 읽기 전용 업무망을 구현했으며, 원천 공동주문·비용 귀속 결속은 아직 만들지 않았다. |
| `PLAN-OPERATIONS-LOGISTICS-OS` | [화물운송 OS와 음식배달 OS r10](Planning/공통/PLAN-OPERATIONS-LOGISTICS-OS/README.md) | `Draft / BackendFoundationImplemented / DisposableMySqlValidated / ShipperToCargoHandoffImplemented / CargoCompletionToShipperAcceptanceImplemented / AbnormalTransportIncidentHoldImplemented / WarehouseOutboundToCargoHandoffImplemented / CompletionProjectionCoreImplemented / OtherConcreteWorkflowAdoptionPending / OperationalActivationDeferred` | 화주 조건 확정과 화물 운송 실행을 별도 OS 책임으로 분리했다. 비정상 운송 사건 원장과 정산 보류 관문까지 결속했으며 재위탁·책임·보상은 자동 확정하지 않는다. |
...
| `PLAN-OPERATIONS-SSALDDEL-MART-SUPPLY-NETWORK` | [살뜰마트 공급망·라스트마일 r1](Planning/공통/PLAN-OPERATIONS-SSALDDEL-MART-SUPPLY-NETWORK/README.md) | `Approved / FirstOperationalSliceImplemented` | 공급계약 이용부터 입고·재고·피킹·포장을 마트 OS가 소유하고, 별도 라스트마일 자식 업무만 음식배달 OS에 인계한다. 40분 내부 목표와 50분 고객 약속 상한을 기준으로 피크·균형·묶음·회복 전략을 결정한다. |
| `PLAN-OPERATIONS-DISPATCH-CORE` | [운영 배차 공통 코어 r41](Planning/공통/PLAN-OPERATIONS-DISPATCH-CORE/README.md) · [구현 준비 점검 r1](Planning/공통/PLAN-OPERATIONS-DISPATCH-CORE/implementation-readiness-audit.r1.md) | `Draft / StakeholderPolicyConfirmedInPart / TwoOperationalOsRelated / FoodWeatherThreePointPolicyApproved / FoodWeatherPickupNowcastFoundationImplemented / FoodWeatherServerCacheImplemented / FoodWeatherThreePointImplementationPending / FreightPostAcceptanceReconsentConfirmed / FreightCommitmentGraphConfirmed / FreightRiskVisibilityConfirmed / FreightInfeasibleReservationReleaseConfirmed / ShipperMaterialChangeOnlyNotificationConfirmed / ShipperRecoveryNotificationConfirmed / FreightContinuityBackendFoundationImplementedDisabled / FreightGraphEffectsDeferred / BackendBatch5InterruptionRecoveryImplemented / FrontendPartial / OperationalActivationDeferred` | 좌표 기반 배달권을 유지하면서 법정동·행정동·기상청 격자를 별도 결속한다. 기사 현재 위치·픽업지·전달지 중 한 곳이라도 유효한 강수가 확인되면 후보별 제안에 플랫폼 부담 건당 1,000원 할증을 한 번만 동결한다. 현재 구현은 픽업지 관측·서버 캐시까지이며 세 지점 판정은 후속 구현 대기다. 배너는 도착만 알리고 기존 지도 상세에서 총 예상 지급액과 할증액을 표시한다. |
...
| `PLAN-SYSTEM-ADMIN-DONG-DIORAMA` | [행정동별 서버 기반 디오라마 r3](Planning/시스템/PLAN-SYSTEM-ADMIN-DONG-DIORAMA/README.md) | `Approved / ActualAdministrativeDongDataGateClosed / AdministrativeDataCollectionFirstSliceStored / ServerProjectionFoundationImplemented / UnityReadOnlyAdapterImplemented / SponsorshipLedgerFoundationImplemented / SceneApplicationDeferred` | 공식 6개 행정동 경계의 공간 투영에 더해 2016 행정동 구역 기준 2026년 7월 생활인구 4,464건을 MySQL 비공개 검토 원장에 저장·재조회했다. 면목제2동·제4동 건물 윤곽 결손은 실제 부재로 해석하지 않는다. Unity Scene·결제는 후속이다. |
| `PLAN-SYSTEM-OBSERVABLE-OPERATIONS-DIORAMA-001` | [사가정역 생활 디오라마 r3](Planning/시스템/PLAN-SYSTEM-OBSERVABLE-OPERATIONS-DIORAMA-001/diorama.r3.md) · [서버 수직 기준 r2](Planning/시스템/PLAN-SYSTEM-OBSERVABLE-OPERATIONS-DIORAMA-001/implementation.r2.md) | `Approved / ImplementationAuthorized / SampleOperationsOnly / OperationalEffectsDisabled` | 기존 여섯 합성 완료 상태 사본을 사가정 1km 동결 지도와 가상 배치 대장에 연결한다. 지도·상태 관찰 우선이며 실제 위치·이동 애니메이션·운영 효과는 제외한다. |
...
| `PLAN-ARCH-OPERATIONS-UNITY-TRANSFER`<!-- compatibility-id: PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001 --> | [운영 서버 0.0~3.5에서 Mirror Unity로의 이관 r8](Planning/시스템/PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001/README.md) · [Unity OS 관찰 모듈 상향식 계획 r4](Planning/시스템/PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001/unity-os-observation-modules.r4.md) · [읽기 전용 관찰 안내](../Architecture/Unity운영데이터읽기전용관찰안내.md) | `ApprovedForHandoff / FoodDeliveryServerVerticalSliceTested / FoodDeliveryUnityMemoryModuleImplemented / ReadOnlyOperationalObservationConfirmed / OperatingSystemCenteredShellConfirmed / RegionalSceneAggregationImplemented / DisposableMySqlAndHttpValidated / UnityReadOnlyClientAndInterpreterImplemented / GameObjectInstantiationDeferred / SimulationWorldShellBindingDeferred / UnityImportDeferred / PresentationE5Blocked` | 첫 표본 `FoodDeliveryOS`의 서버 정상 생명주기와 완료 사본 조회, 인증 GET→Interpreter→OS별 메모리 Router·음식배달 관찰 Adapter까지 고정했다. 다음은 검증된 공간 결속과 primitive 표현이며 Prefab은 그 뒤다. |
