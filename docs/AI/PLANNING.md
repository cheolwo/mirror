...
| 기획 ID | 현재 문서 | 상태 | 역할 |
| --- | --- | --- | --- |
| `PLAN-OPERATIONS-LOGISTICS-OS` | [화물운송 OS와 음식배달 OS r6](Planning/공통/PLAN-OPERATIONS-LOGISTICS-OS/README.md) | `Draft / BackendFoundationImplemented / DisposableMySqlValidated / WarehouseOutboundToCargoHandoffImplemented / CompletionProjectionCoreImplemented / OtherConcreteWorkflowAdoptionPending / OperationalActivationDeferred` | 두 OS의 생명주기·엔진 대장·멱등 인계 기반과 창고→화물 첫 인계를 구현했다. 공통 완료 증명으로 음식·창고·화물 완료를 지역 장면 자료원으로 조합하고 일회용 MySQL·실제 HTTP를 검증했으며 운영 활성화는 후속이다. |
...
| `PLAN-ARCH-OPERATIONS-UNITY-TRANSFER`<!-- compatibility-id: PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001 --> | [운영 서버 0.0~3.5에서 Mirror Unity로의 이관 r8](Planning/시스템/PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001/README.md) | `ApprovedForHandoff / RegionalSceneAggregationImplemented / DisposableMySqlAndHttpValidated / UnityReadOnlyClientAndInterpreterImplemented / GameObjectInstantiationDeferred / SimulationWorldShellBindingDeferred / UnityImportDeferred / PresentationE5Blocked` | 완료가 증명된 음식·창고·화물 자료원을 비식별 지역 장면으로 조합하고, Unity GET Client·JSON 해석·cursor/TTL/tombstone 메모리 반영을 구현했다. 일회용 MySQL과 실제 인증 HTTP는 검증했으나 제품 Unity 프로젝트가 없어 Scene·Play Mode는 열지 않았다. |
