...
| 기획 ID | 현재 문서 | 상태 | 역할 |
| --- | --- | --- | --- |
| `PLAN-OPERATIONS-LOGISTICS-OS` | [화물운송 OS와 음식배달 OS r5](Planning/공통/PLAN-OPERATIONS-LOGISTICS-OS/README.md) | `Draft / TwoOperationalOsConfirmed / EndToEndLifecycleOwnershipConfirmed / InternalResponsibilitySplitConfirmed / SharedDispatchCoreRelated / ExplicitCrossOsHandoffConfirmed / PerOsEngineCatalogConfirmed / BackendFoundationImplemented / DatabaseMigrationPreparedNotApplied / WarehouseOutboundToCargoHandoffImplemented / OtherConcreteWorkflowAdoptionPending / FallbackActivationPending / OperationalActivationDeferred` | 두 OS의 전 생명주기 대장과 OS별 Engine Catalog, 멱등 인계 원장·Outbox 기반을 구현했다. 창고 출고 완료에서 기존 화물 의뢰로 넘어가는 첫 명시적 인계는 연결했으며 다른 ProcessManager와 승인된 fallback·운영 활성화는 후속이다. |
| `PLAN-OPERATIONS-DISPATCH-CORE` | [운영 배차 공통 코어 r35](Planning/공통/PLAN-OPERATIONS-DISPATCH-CORE/README.md) · [구현 준비 점검 r1](Planning/공통/PLAN-OPERATIONS-DISPATCH-CORE/implementation-readiness-audit.r1.md) | `Draft / StakeholderPolicyConfirmedInPart / TwoOperationalOsRelated / FreightPostAcceptanceReconsentConfirmed / FreightCommitmentGraphConfirmed / FreightRiskVisibilityConfirmed / FreightInfeasibleReservationReleaseConfirmed / ShipperMaterialChangeOnlyNotificationConfirmed / ShipperRecoveryNotificationConfirmed / FreightContinuityBackendFoundationImplementedDisabled / FreightGraphEffectsDeferred / BackendBatch5InterruptionRecoveryImplemented / FrontendPartial / OperationalActivationDeferred` | 화물운송 OS와 음식배달 OS의 상위 실행자가 아니라 두 OS가 공유하는 유효 제안·수락·거절·중단·지표·revision·감사 기반을 소유한다. 화물 운송 약속과 음식 조리·재조리 같은 고유 의미는 각 OS가 소유한다. |
...
| `PLAN-ARCH-OPERATIONS-UNITY-TRANSFER`<!-- compatibility-id: PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001 --> | [운영 서버 0.0~3.5에서 Mirror Unity로의 이관 r7](Planning/시스템/PLAN-ARCH-OPERATIONS-UNITY-TRANSFER-001/README.md) | `ApprovedForHandoff / InventoryImplemented / AppObservationProfilesSeparated / OperationalRoleObjectCatalogImplemented / GameObjectInstantiationDeferred / OperationalSimulationWorkflowInterfacesSeparated / SharedPureCoreConfirmed / SimulationWorkflowBoundaryImplemented / OperationalClientBoundaryImplemented / UnityReadOnlyTransportCentralized / FoodDeliveryCompletedLifecycleProjectionImplemented / ProjectionDatabaseMigrationPreparedNotApplied / SimulationWorldShellBindingDeferred / LegacyFacadeRetained / UnityImportDeferred / LogisticsMirrorGoalBound / HubFirstSliceApproved / PresentationE5Blocked` | Web·MAUI는 운영 서버 API Client를 명시적으로 사용하고 Unity 운영 관찰은 공통 GET 전송만 사용한다. FoodDeliveryOS 정상 완료 건은 같은 DB 저장 단위의 발행 요청, 비동기 검증, 비식별·온라인 전용 지역 상태 사본 조회까지 첫 수직 조각을 구현했다. 실제 Unity Scene 생성은 열지 않았다. |
