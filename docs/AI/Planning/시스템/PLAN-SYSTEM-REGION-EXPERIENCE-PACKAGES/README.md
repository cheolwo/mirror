[기획 · 시스템·월드 투영 · PLAN-SYSTEM-REGION-EXPERIENCE-PACKAGES · r1]

# Steam 무료 본편의 지역 Experience Package

상태: `Approved / FirstServerContractImplemented / SagajeongPrivatePreviewRegistered / UnityStreamingDeferred`
...
Steam 무료 본편
  ...
  → 행정동 지리 / 생활 맥락 / 복구 시나리오 / gameplay / 상업 표시 / 운영 상태
```
...
- 사가정 패키지는 기존 행정동 manifest/tile, display overlay, 운영 장면 v2를 참조한다. 생활 맥락·초기 복구 정착기·gameplay는 endpoint 없는 `Planned`로 남긴다.
- 기능 플래그 `RegionExperiencePackages`는 기본 비활성이다. 기존 행정동·운영 관찰 플래그가 켜진 경우에만 해당 레이어가 `Ready`와 endpoint를 노출한다.
- 이번 절편은 RDB·MongoDB·Simulation·Unity Scene·Steamworks·다운로드 CDN을 변경하지 않는다.

## 후속 순서

1. Unity 공용 Package Client·Mapper·hash 캐시와 필수 지리 bootstrap을 구현한다.
2. 500m 타일을 on-demand로 읽고 카메라 주변 예산에 따라 조립·해제한다.
...
공유 계약·서버 조립·기능 플래그·ETag·기존 route 참조까지만 구현 증거다. 지역 다운로드 파일, Steam 배포, Unity 스트리밍, 프레임 개선, 실제 광고·서비스와 공개 출시는 아직 구현·검증되지 않았다.

## 다음 질문 하나

없음. 다음 구현은 새 지역 추가가 아니라 사가정 패키지의 Unity bootstrap/on-demand 캐시 절편이다.
