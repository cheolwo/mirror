# [기획 · 월드·공간·배치 · PLAN-SYSTEM-ADMIN-DONG-DIORAMA · r3]
...
- 경계별 현실 자료는 원천의 공간 판본을 보존한다. 서울시가 2016 행정동 구역으로 사전 집계한 생활인구와 2026년 현행 경계에 좌표를 판정한 건물·사업장을 같은 공간 정밀도로 간주하거나 합산하지 않는다.
...
- 서울시 `OA-14991` 2026년 7월 완결본에서 6개 동별 744시간, 합계 4,464개의 총생활인구 추정치를 수집해 로컬 MySQL 비공개 검토 원장에 저장했다. 동일 입력 재적용은 신규 0·기존 4,464이고 독립 재조회도 통과했다. [수집·분석과 결손 대조](../../../../Reports/면목동-행정동별-생활인구-수집분석-2026-09-12.md)에 따라 면목제2동의 건물 후보 0건은 실제 부재가 아니라 현행 공간 원본 범위 결손으로 유지한다.
...
- 현행 경계 기준 주민등록 인구·세대, 면목제2동·제4동 건물 윤곽, 생활 시설 자료. 최신 250m 생활인구는 별도 격자 교차 집계 판본이 마련되기 전까지 2016 행정동 구역 자료와 결합하지 않는다.

## 검증 상한

`ActualAdministrativeDongDataGate=Closed`다. 공식 경계·6개 행정동 코드 관할·건물 602개 전수 귀속·도로 2,397개 입력·상권 관측 5,411개 좌표 분석·면목제3·8동 Mongo 판본·귀속 감사자료의 재적용과 독립 재조회가 공간 관문의 증거 범위다. 별도로 2016 행정동 구역 기준 생활인구 4,464건을 MySQL에 비공개 저장·재적용·독립 재조회했다. 이는 경계별 분석 자료의 첫 조각이지 현행 경계 기준 모든 자료의 완결 선언은 아니다. Unity Scene·실제 Game View, RDB 표시 원장 migration 적용, 운영 서버 HTTP, 광고/후원 표시는 아직 검증하지 않았다.

## 다음 질문 하나

경계별 다음 수집은 `현행 주민등록 인구·세대`와 `면목제2동·제4동 건물 윤곽 보강` 중 어느 쪽을 우선할 것인가? 추천은 후자다. 기능 생활권의 공간 결손을 먼저 줄일 수 있지만, 공식 원본의 이용조건과 전수 범위를 확인하는 비용이 더 크다. 광고·후원 표시는 Claim 검증과 별도 운영 승인이 생기기 전까지 0건을 유지한다.
